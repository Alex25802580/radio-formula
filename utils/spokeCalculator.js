...
function validateErd(value) {
  return validateMeasurement(value, { label: 'ERD', ...LIMITS.erd });
}

function validatePcd(value, side) {
...
function validateFlangeDistance(value, side) {
  return validateMeasurement(value, {
...
function validateRimOffset(value) {
  return validateMeasurement(value, {
    label: 'Rim offset',
    ...LIMITS.rimOffset,
  });
...
function validateWheel(wheel) {
  ...
  if (wheel.crosses > maxCrosses) {
    return {
      valid: false,
      message: `Too many crosses for a ${wheel.holes}-hole wheel.`,
    };
  }
...
function calculateSpokeLength({ erd, pcd, flangeDistance, crosses, holes }) {
...
function calculateWheelSpokes(wheel) {
